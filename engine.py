...
def evaluate_performance(
    epoch, model, gallery_loader, query_loader, device, use_gt=False, use_cache=False, use_cbgm=False
):
    """
    Args:
    ...
    content = "epoch:{}\n".format(epoch)
    det_rate, ap=eval_detection(gallery_loader.dataset, gallery_dets, det_thresh=0.01)
    content = content + "all detection:\n"
    content = content + "  recall = {:.2%}\n".format(det_rate)
    content = content + "  ap = {:.2%}\n\n".format(ap)
    eval_search_func = (
    ...
    content = content + "search ranking:\n"
    content = content + "  mAP = {:.2%}\n".format(ret["mAP"])
    for i, k in enumerate([1, 5, 10]):
        content = content + "  top-{:2d} = {:.2%}\n".format(k, ret["accs"][i])
