...
class DTHN(nn.Module):
    def __init__(self, cfg):
        super(DTHN, self).__init__()
...
class DTHRoIHeads(RoIHeads):
    def __init__(
        ...
        super(DTHRoIHeads, self).__init__(*args, **kwargs)
        embedding_dim = 256
        self.embedding_head = NormAwareEmbedding(featmap_names=["before_trans", "after_trans"],
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        box_regs = self.box_predictor(box_features["after_trans"])
