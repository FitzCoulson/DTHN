...
class TRHN(nn.Module):
    def __init__(self, cfg):
        super(TRHN, self).__init__()
        backbone, box_head = build_resnet(name="resnet50", pretrained=True)
        reid_head = TransformerHead(
            cfg=cfg,
...
class TRHRoIHeads(RoIHeads):
    def __init__(
        ...
        super(TRHRoIHeads, self).__init__(*args, **kwargs)
        embedding_dim = 256
